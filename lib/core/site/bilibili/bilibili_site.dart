...
class BiliBiliSite
  ...
  Future<Map<String, String>> getHeader() async {
    ...
    final cookieBuvid3 =
        RegExp(r'(?:^|;)\s*buvid3=([^;]+)').firstMatch(storedCookie)?.group(1) ?? '';
    final cookieBuvid4 =
        RegExp(r'(?:^|;)\s*buvid4=([^;]+)').firstMatch(storedCookie)?.group(1) ?? '';
    ...
    return storedCookie.isEmpty
        ...
        : {
            'cookie': storedCookie.contains('buvid3')
                ? storedCookie
                : '$storedCookie;buvid3=$buvid3;buvid4=$buvid4;',
            'user-agent': kDefaultUserAgent,
            'referer': kDefaultReferer,
          };
  }
  ...
  /// applied.
  static LivePlayUrlResolution parsePlayUrlResolution(
    dynamic response, {
    required Object? requestedQualityData,
  }) {
    final playUrl = _playUrlPayload(response);
    ...
    candidates.sort((a, b) {
      final requestedOrder = (b.currentQn == requestedQn ? 1 : 0).compareTo(
        a.currentQn == requestedQn ? 1 : 0,
      );
  ...
  Future<(String, String)> getWbiKeys({bool forceRefresh = false}) async {
    final cacheAge = _wbiKeysUpdatedAt == null
        ? null
        : DateTime.now().difference(_wbiKeysUpdatedAt!);
    if (!forceRefresh &&
        kImgKey.isNotEmpty &&
        kSubKey.isNotEmpty &&
        cacheAge != null &&
    ...
    for (var attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        final signed = await getWbiSign(
          ...
          forceRefresh: attempt == 1 || attempt == 3,
        );
        ...
        if (response['code'] == 0 &&
            candidate is Map &&
            candidate['token']?.toString().isNotEmpty == true) {
  ...
  Future<List<LiveRoom>> searchRooms(String keyword, {int page = 1, int pageSize = 30}) async {
    ...
    for (var item in queryList ?? []) {
      ...
      var roomItem = LiveRoom(
        ...
        audienceMetricType: AudienceMetricType.popularity,
        liveStatus: int.tryParse(item['live_status']?.toString() ?? '') == 1
            ? LiveStatus.live
            : LiveStatus.offline,
