...
/// 沉浸式播放页布局：视频铺满全屏，侧栏默认隐藏，鼠标移入右缘触发区自动
...
class _LivePlayShellState extends State<LivePlayShell> with SingleTickerProviderStateMixin {
            ...
            child: IgnorePointer(
              ...
              child: MouseRegion(
                ...
                child: SlideTransition(
                  ...
                  ).animate(CurvedAnimation(parent: _drawerController, curve: Curves.easeInOutCubic)),
  ...
  Widget _buildOpacityControl() {
    ...
    final fillFraction =
        ((panelOpacity - PanelSizeController.kMinImmersiveOpacity) /
                (PanelSizeController.kMaxImmersiveOpacity - PanelSizeController.kMinImmersiveOpacity))
            .clamp(0.0, 1.0);
    return Listener(
  ...
  bool _isInsideTriggerRect(Offset local) {
    final size = _shellSize();
    if (size.isEmpty) return false;
    return local.dx >= size.width - 80 && local.dx <= size.width && local.dy >= 60 && local.dy <= size.height - 100;
  }
