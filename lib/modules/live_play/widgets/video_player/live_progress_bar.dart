...
/// Coordinate system
/// -----------------
/// The adapter remaps all fractions to the *cached seekable window*
/// [earliestCachedPosition, duration], so the visible track always spans
/// exactly the still-buffered range. Content evicted from the back buffer is
/// simply not drawn — the track left edge corresponds to the oldest cached
/// frame, the right edge to the current live position. The left-side time
/// label shows the total elapsed time since stream opened (liveEdgePosition),
/// which grows monotonically regardless of eviction.
///
/// Seeking to positions outside the cached window is impossible because the
/// fraction space has already been compressed to the window; the adapter's
/// [seekToFraction] converts back to absolute Duration and [_clampToSeekable]
/// enforces the earliest boundary as a final guard.
...
class _LiveProgressBarState extends State<LiveProgressBar> {
  ...
  /// Label value shown left of the bar (e.g. "30:00"). This is the total
  /// elapsed time since the stream opened (liveEdgePosition), so it grows
  /// monotonically even when the back buffer evicts old content.
  ...
  void _tick() {
    ...
    if (!player.canSeek) {
      if (_currentFrac != 0.0 || _ranges.isNotEmpty || _windowLabel != Duration.zero || _userSeeked) {
        setState(() {
          _currentFrac = 0.0;
          _ranges = const [];
          _windowLabel = Duration.zero;
    ...
    // Left label = total elapsed time since stream opened, always monotonic.
    final label = player.liveEdgePosition;
  ...
  static bool _rangesEqual(List<({double start, double end})> a, List<({double start, double end})> b) {
...
class _LiveProgressPainter extends CustomPainter {
  ...
  static final Color _playedSeekedColor = Colors.white.withValues(alpha: 0.92);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 1. Background track — the full visible timeline (already remapped to
    //    the [earliest, duration] window by the adapter).
    canvas.drawLine(Offset(0, y), Offset(w, y), trackPaint);

    // No cached ranges (VOD or a just-started live stream): the whole
    // track is seekable, so draw it as buffered with the played portion
    // on top.
    if (ranges.isEmpty) {
      _segment(canvas, y, w, h, 0.0, 1.0, bufferedPaint);
      if (frac > 0.0) _segment(canvas, y, w, h, 0.0, frac, playedPaint);
      return;
    }
