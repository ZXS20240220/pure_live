...
class MultiviewRoomSearchController {
  MultiviewRoomSearchController({List<Site>? sites, int Function(LiveRoom left, LiveRoom right)? audienceCompare})
  ...
  Future<LiveRoom?> resolveDirect(String input, {String platformId = ''}) async {
    ...
    try {
      return await liveSite.getRoomDetail(platform: platform, roomId: roomId).timeout(liveSearchRequestTimeout);
    } catch (error) {
