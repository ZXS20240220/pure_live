...
部分平台的未开播/预热房间不会返回开播时间，导致按开播时间排序时这些房间被混排到末尾或位置不稳定。新增一套"假开播时间"计算逻辑：对有开播状态但缺失真实开播时间的房间，用**应用拉取关注列表的当前时刻**作为其开播时间，保证所有在线房间都能获得一个用于排序的时间值，但不会影响在程序启动前就已经开播的直播间。真实开播时间返回后会自动覆盖假值，房间结束直播时该假值被清理。
...
- **新增历史记录单个删除功能**：长按/右键某条历史记录，可单独移除，不再只能全部清空
- 实时监听收藏变更、房间切换、历史变动等事件，自动刷新列表
- 历史房间同步收藏夹的实时直播状态，过滤离线房间
- 多画面选台面板复用该逻辑，同样同步收藏状态并过滤离线房间
...
Super Chat 面板顶部新增 🔄 刷新按钮，点击后重新拉取当前直播间的醒目留言和 AI 看点数据，不必手动重新进入直播间。
